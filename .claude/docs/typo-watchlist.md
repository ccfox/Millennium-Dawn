# Recurring Typo Watchlist

Check for these when reviewing localisation files:

| Typo                                                   | Correct                                 |
| ------------------------------------------------------ | --------------------------------------- |
| `Estabilish` / `estabilish`                            | `Establish` / `establish`               |
| `innvoations`                                          | `innovations`                           |
| `irreperable` / `irrepairable` / `unrepairable`        | `irreparable`                           |
| `unenmployed`                                          | `unemployed`                            |
| `existance`                                            | `existence`                             |
| `effectivness`                                         | `effectiveness`                         |
| `disproportinate`                                      | `disproportionate`                      |
| `tarditions`                                           | `traditions`                            |
| `contrats` (used as contrast)                          | `by contrast`                           |
| `Airforce`                                             | `Air Force`                             |
| `miltiary`                                             | `military`                              |
| `coaltion`                                             | `coalition`                             |
| `tumultous`                                            | `tumultuous`                            |
| `recgonized`                                           | `recognized`                            |
| `Propgramme`                                           | `Programme`                             |
| `poeple`                                               | `people`                                |
| `it's` (possessive)                                    | `its`                                   |
| `Unloyal`                                              | `Disloyal`                              |
| `Isreal`                                               | `Israel`                                |
| `bocme`                                                | `become`                                |
| `hovewer`                                              | `however`                               |
| `acomplish`                                            | `accomplish`                            |
| `Endevours`                                            | `Endeavours`                            |
| `Quiantified`                                          | `Quantified`                            |
| `convering`                                            | `converting`                            |
| `encomapassing`                                        | `encompassing`                          |
| `fundamnetals`                                         | `fundamentals`                          |
| `civillian`                                            | `civilian`                              |
| `civillisation` / `civilisation` (in American English) | `civilization`                          |
| `suprised`                                             | `surprised`                             |
| `alledged`                                             | `alleged`                               |
| `succesful` / `succesfull`                             | `successful`                            |
| `huminliating`                                         | `humiliating`                           |
| `reffered`                                             | `referred`                              |
| `stronly`                                              | `strongly`                              |
| `togeather`                                            | `together`                              |
| `disasterous`                                          | `disastrous`                            |
| `religous`                                             | `religious`                             |
| `suzerainity`                                          | `suzerainty`                            |
| `seperation` / `seperate` / `seperated`                | `separation` / `separate` / `separated` |
